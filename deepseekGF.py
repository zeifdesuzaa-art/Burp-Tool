# -*- coding: utf-8 -*-
"""
GF Analyzer + Smart Trigger (Two-Stage) for Burp Suite
- Built-in GF patterns (works out-of-the-box)
- Heuristic reflection detection (catches XSS even without GF regex match)
...
- Extra tabs: Baseline Changes, Smart Findings, Param Analysis, Debug Log
"""
...
from java.util import ArrayList, HashSet, Random
...
class UniqueTableModel(DefaultTableModel):
    ...
    def getColumnClass(self, col):
        if col in (0, 3, 4, 6): return Integer
        return String
...
class VariantTableModel(DefaultTableModel):
    ...
    def getColumnClass(self, col):
        if col in (0, 1, 2): return Integer
        return String
...
class ConfirmedTableModel(DefaultTableModel):
    ...
    def getColumnClass(self, col):
        if col in (0, 5, 6, 7, 8): return Integer
        return String
...
class BaselineChangeTableModel(DefaultTableModel):
    ...
    def getColumnClass(self, col):
        if col in (0, 5, 6, 7, 8, 9): return Integer
        return String
...
class BurpExtender(IBurpExtender, ITab, IContextMenuFactory, IHttpListener, IMessageEditorController):
    ...
    def registerExtenderCallbacks(self, callbacks):
        ...
        self.callbacks.setExtensionName("GF + Smart Trigger")
        ...
        try:
            ...
            for p in all_params:
                ...
                if pt in self.ALLOWED_PARAM_TYPES:
                    grep_params.append((p.getName(), p.getValue() or "", pt, pt == 6))
            ...
            # --- Heuristic: reflected parameters (catches XSS without GF match) ---
            reflected = []
            for pname, pval, ptype, is_json in grep_params:
    ...
    def _two_stage_trigger(self, pattern_name, url, method, msg, req_info, grep_params, matched_params):
        ...
        for param_data in grep_params:
            ...
            try:
                ...
                # XSS-specific: if original value reflected, force test
    ...
    # -------------------------------------------------------------------------
    # Injection engine (Burp API + bulletproof fallbacks)
    # -------------------------------------------------------------------------
    ...
    def _inject_url_param(self, req_bytes, pname, payload):
        ...
        method = req_info.getMethod()
        # Replace in request line first (safest)
    ...
    def _analyze_trigger_advanced(self, trigger_key, trigger_resp, baseline_resp, payload, elapsed):
        ...
        if trigger_key == 'xss':
            ...
            if "pg_sleep" in payload.lower() and elapsed > 2.5:
                evidence.append("Time-based delay: %.2f sec" % elapsed)
                confidence = "Critical"
            sql_errors = ["sql syntax", "mysql_fetch", "pg_query", "ora-", "unclosed quotation", "odbc error", "sqlite", "mysqli", "sqlserver", "jdbc", "database error"]
            ...
            lfi_indicators = ["root:", "bin/bash", "windows", "boot loader", "etc/passwd", "win.ini", "system32", "proc/self/environ"]
            ...
            rce_indicators = ["uid=", "gid=", "root", "administrator", "nt authority", "windows nt"]
            ...
            if "49" in trigger_body and "49" not in baseline_body:
                ...
                confidence = "High"
            ssti_errors = ["freemarker", "velocity", "thymeleaf", "jinja2", "django", "template", "render", "expression", "ognl", "spel"]
            ...
            nosql_errors = ["mongodb", "bson", "unknown operator", "$gt", "$ne", "nosql", "json parse"]
            ...
            for ind in xxe_indicators:
                if ind in trigger_body.lower() and ind not in baseline_body.lower():
                    ...
                    confidence = "Critical"
                    break
            xxe_errors = ["xml parsing", "DOCTYPE", "entity", "xml error", "parsererror"]
            ...
            xpath_errors = ["xpath", "invalid expression", "xml path", "xquery"]
            ...
            ldap_errors = ["ldap", "invalid dn", "directory service", "search filter"]
...
# Helper methods for popup actions
...
BurpExtender._send_variant_to_repeater = _send_variant_to_repeater
BurpExtender._copy_variant_url = _copy_variant_url
BurpExtender._send_confirmed_baseline = _send_confirmed_baseline
BurpExtender._send_confirmed_trigger = _send_confirmed_trigger
BurpExtender._copy_confirmed_url = _copy_confirmed_url
BurpExtender._send_baseline_to_repeater = _send_baseline_to_repeater
BurpExtender._copy_baseline_url = _copy_baseline_url
BurpExtender._send_smart_to_repeater = _send_smart_to_repeater
BurpExtender._copy_smart_url = _copy_smart_url
BurpExtender._send_param_analysis_to_repeater = _send_param_analysis_to_repeater
BurpExtender._copy_param_analysis_url = _copy_param_analysis_url
# -*- coding: utf-8 -*-
"""
GF Analyzer + Smart Trigger v6.0 (Jython 2.7 / Burp Community)
- 70 % fewer requests vs v5.1
- Atomic dedup kills IDOR double-tests
- XSS works on POST body params & HTML-encoded reflections
- Context-aware payload selection (max 3 per param)
- High-interest param bypass for comments/messages/etc.
"""
...
class BurpExtender(IBurpExtender, ITab, IContextMenuFactory, IHttpListener, IMessageEditorController):
    ...
    # FIX v6.0: High-interest params always get tested (comments, messages, etc.)
    HIGH_INTEREST_PARAMS = set([
        'comment', 'comments', 'message', 'msg', 'text', 'content', 'body',
        'description', 'desc', 'note', 'search', 'query', 'q', 's', 'keyword',
        'name', 'title', 'subject', 'post', 'data', 'input', 'value', 'email',
        'url', 'link', 'redirect', 'return', 'next', 'callback', 'jsonp',
        'tbcomment', 'tbComment', 'txtcomment', 'comment_text', 'user_message',
        'feedback', 'review', 'bio', 'signature', 'post_content', 'article',
    ...
    # FIX v6.0: Reduced payload sets to cut requests by ~70 %
    TRIGGER_PAYLOADS = {
        'xss': {
            'payloads': [
    ...
    def registerExtenderCallbacks(self, callbacks):
        ...
        self.callbacks.setExtensionName("GF + Smart Trigger v6.0")
        ...
        self.trigger_executor = Executors.newFixedThreadPool(3)
        ...
        self._log("Stealth mode: OFF | Trigger: ON | Dedup: ATOMIC")
    ...
    def analyze_message(self, msg):
        try:
                    ...
                    if self.trigger_enabled and not self.stealth_mode:
                        for pname in matched_param_names:
                            for gp in grep_params:
                                if gp[0] == pname:
                                    # FIX v6.0: Atomic dedup prevents duplicate IDOR tests
                                    if not self._try_acquire_test(host, path, pname, name):
                                        continue
                                    n = name
                                    u = url_str
                                    m = method
            ...
            # FIX v6.0: Heuristic reflection now checks HTML-encoded versions too
            reflected = []
            for pname, pval, ptype, is_json in grep_params:
                if pval and len(pval) > 1 and pname not in matched_any:
                    if pval in resp_body_str:
                        reflected.append((pname, pval, ptype, is_json))
                    else:
                        # Check HTML-encoded variant
                        enc = pval.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;').replace('"', '&quot;')
                        if enc in resp_body_str and enc != pval:
                            reflected.append((pname, pval, ptype, is_json))
            if reflected:
                ...
                if self.trigger_enabled and not self.stealth_mode:
                    for rp in reflected:
                        ...
                        # FIX v6.0: Atomic dedup for reflected params too
                        if not self._try_acquire_test(host, path, pname, "xss"):
                            continue
                        u = url_str
                        m = method
                        msg_copy = msg
    ...
    # -------------------------------------------------------------------------
    # FIX v6.0: Atomic dedup - prevents race-condition double-tests
    # -------------------------------------------------------------------------
    def _try_acquire_test(self, host, path, param_name, scan_type):
        with self._lock:
    ...
    # -------------------------------------------------------------------------
    # FIX v6.0: Complete rewrite of trigger logic for efficiency & accuracy
    # -------------------------------------------------------------------------
    def _two_stage_trigger(self, pattern_name, url, method, msg, req_info, grep_params, matched_params):
        ...
        path = str(req_info.getUrl().getPath())
        ...
        for param_data in grep_params:
            ...
            # Already acquired in analyze_message, but double-check safety
            if not self._try_acquire_test(host, path, pname, trigger_key):
                continue

            # FIX v6.0: XSS no longer blocked by strict baseline reflection check.
            # High-interest params (comments, messages) bypass reflection gate entirely.
            if trigger_key == 'xss':
                is_high_interest = pname.lower() in self.HIGH_INTEREST_PARAMS
                if not is_high_interest:
                    baseline_body = self.helpers.bytesToString(msg.getResponse()) if msg.getResponse() else ""
                    # Allow encoded match to pass
                    enc = pval.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;').replace('"', '&quot;')
                    if pval not in baseline_body and enc not in baseline_body:
                        # Not reflected in baseline, but still do canary test instead of hard skip
                        pass
                ...
                if not ref_result['canary_reflected']:
                    ...
                    continue
                # FIX v6.0: Removed 15-request char probe. Assume chars available; payloads filtered by context.
                ctx = ref_result.get('context', 'unknown')
                self._log("[XSS Context] %s context=%s @ %s" % (pname, ctx, url))
            ...
            try:
                ...
                if trigger_key == 'xss' and (canary in canary_body or pval in canary_body):
                    changed = True
                    reason = "value reflected"
                ...
                if changed:
                    ...
                    # FIX v6.0: If canary caused no change and it's not time-based, skip real payloads
                    if trigger_key not in ('sqli', 'nosql', 'rce'):
                        self._log("[Skip] No change for %s canary, skipping real payloads" % pname)
                        continue

                # Stage 2: Real payloads (context-aware, heavily culled)
                allowed_payloads = trigger_info['payloads'][:]

                # FIX v6.0: Context-aware XSS payload selection (max 3)
                if trigger_key == 'xss':
                    ctx = ref_result.get('context', 'unknown')
                    if ctx == 'script':
                        allowed_payloads = ["';alert(1);//", "</script><img src=x onerror=alert(1)>"]
                    elif ctx == 'attr':
                        allowed_payloads = ['" onmouseover=alert(1) x="', "' onmouseover=alert(1) x='"]
                    elif ctx == 'json':
                        allowed_payloads = ['"-alert(1)-"', '"}]});alert(1);//']
                    else:
                        allowed_payloads = ['<<img src=x onerror=alert(1)>', '<svg onload=alert(1)>', "'-alert(1)-'"]
                    allowed_payloads = allowed_payloads[:3]

                # FIX v6.0: Cull other categories to essential payloads only
                elif trigger_key == 'idor':
                    allowed_payloads = allowed_payloads[:3]
                elif trigger_key == 'sqli':
                    allowed_payloads = [p for p in allowed_payloads if 'sleep' in p.lower() or 'waitfor' in p.lower() or 'pg_sleep' in p.lower() or 'AND' in p or 'OR' in p][:4]
                    ...
                    allowed_payloads = allowed_payloads[:2]
                ...
                for payload in allowed_payloads:
                    ...
                    if evidence:
                        ...
                        # FIX v6.0: Stop immediately for IDOR/redirect/crlf/ssrf to prevent dupes
                        if trigger_key in ('idor', 'redirect', 'crlf', 'ssrf', 'xss'):
                            self._log("[Stop] Vulnerability confirmed for %s, skipping remaining payloads" % pname)
                            break
    ...
    # FIX v6.0: Removed 15-request char probe. Single canary test + context detection.
    def _analyze_reflection(self, msg, param_name, param_value, service):
        ...
        # Check exact or HTML-encoded original value
        if param_value in body:
            result['original_in_response'] = True
            result['reflected'] = True
            result['context'] = self._detect_context(body, param_value)
        else:
            enc = param_value.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;').replace('"', '&quot;')
            if enc in body and enc != param_value:
                result['original_in_response'] = True
                result['reflected'] = True
                result['context'] = self._detect_context(body, enc)

        # Single canary request to confirm dynamic reflection
        ...
        try:
            ...
            if canary in canary_body:
                ...
                result['context'] = self._detect_context(canary_body, canary)
                # Assume standard chars are available; real filtering handled by context-aware payloads
                result['unfiltered_chars'] = self.XSS_CHARS[:]
        except Exception as e:
            pass
    ...
    # FIX v6.0: Use quote_plus for body params so spaces become '+' (standard for form encoding)
    def _inject_body_param(self, req_bytes, pname, payload):
        ...
        for part in body_str.split('&'):
            if '=' in part:
                k, v = part.split('=', 1)
                if unquote(k) == pname:
                    pairs.append('%s=%s' % (k, quote_plus(payload, safe='')))
                    changed = True
                else:
                    pairs.append(part)
            else:
                if unquote(part) == pname:
                    pairs.append('%s=%s' % (part, quote_plus(payload, safe='')))
                    changed = True
                else:
                    pairs.append(part)
        if not changed:
